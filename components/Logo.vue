...
export default {
  ...
  methods :{
    async fetchXXX() {
      ...
      this.mountains = await fetch('https://opencms.codes/api/?name=mountains&wait=2').then(res => res.json());
      this.planets = await fetch('https://opencms.codes/api/?name=planets&wait=5').then(res => res.json());
